(function (moment) {
  ...
  var template = '<md-dialog class="dtp" layout="column" style="width: 300px;">'
    ...
    + '                <div class="dtp-actual-day">{{picker.currentDate.format("dddd")}}, {{picker.currentDate.format("MMMM")}} {{picker.currentDate.format("DD")}}, {{picker.currentDate.format("YYYY")}} - {{picker.currentNearest5Minute().format(picker.params.shortTime ? "hh:mm" : "HH:mm")}} </div>'
    ...
    + '                <div layout="column">'
    + '                    <div class="dtp-actual-month">{{picker.currentDate.format("dddd")}}, {{picker.currentDate.format("MMMM")}} {{picker.currentDate.format("DD")}}</div>'
    + '                </div>'
    + '                <div layout="row">'
    ...
    + '            <md-button id="ChangeDate" class="dtp-btn-ok md-button" ng-click="picker.ok()"> {{picker.params.okText}}</md-button>'
    + '      </md-dialog-actions>'
    + '</md-dialog>';
  ...
  PluginController.prototype = {
    ...
    hide: function (okBtn) {
      if (okBtn) {
        ...
        angular.element('#Data_Cons').trigger('change');
    }
  };
