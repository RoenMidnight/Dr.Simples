 ...
 //   var markerCluster;
    ...
    function setMarkers(cidad, espec, medic, filter) {
                        ...
                        (function (i) {
                            ...
                            $.ajax({
                                ...
                                success: function (results, status) {
                                    ...
                                    if (status == "success" && results.status != "ZERO_RESULTS") {

                                        var searchUrl;

                                        if (filter == "Medico") {
                                            searchUrl = "../Paciente/Confirma/" + address[i].id + "?redirect=false"
                                        } else {
                                            searchUrl = "../Paciente/SelectDoc/" + address[i].id
                                        }

                                        var latitude = results.results[0].geometry.location.lat;
                                        var lngitude = results.results[0].geometry.location.lng;

                                        if (markers.length != 0) {
                                            for (y = 0; y < markers.length; y++) {
                                                var exMaker = markers[y];
                                                var laTeste = exMaker.getPosition().lat();
                                                var loTeste = exMaker.getPosition().lng();
                                                
                                                if (laTeste.toString().substring(0,10) == latitude.toString().substring(0, 10) &&
                                                    loTeste.toString().substring(0,10) == lngitude.toString().substring(0, 10)) {
                                                    latitude = latitude + (Math.random() -.5) / 1500;
                                                    lngitude = lngitude + (Math.random() -.5) / 1500;
                                                }
                                        ...
                                        if (filter == "Medico") {
                                            var content = "<p id=markerMed>" + address[i].medic + "</p><p id=markerConsult>" + address[i].consult +
                                                          "</p><p><a href="  + searchUrl + ">Marcar Consulta</a></p>";
                                        } else {
                                            var content = "<p id=markerMed>" + address[i].medic + "</p><p id=markerConsult>" + address[i].consult +
                                                          "</p><p><a href='" + searchUrl + "' data-target='.inbox-body' id='markModal'>Marcar Consulta</a></p>";
                                        }
                                        ...
                                        if (filter != "Medico") {
                                            ...
                                            google.maps.event.addListener(marker, 'click', function () {
                                                return function () {
                                                    ...
                                                    $('#markModal').on('click', function (e) {
                                                        e.preventDefault();
                                                       
                                                        $('.inbox-body').load(this.href);
                                                        $("#myModal").modal();
                                                    });
        ...
        markerClusterer = new MarkerClusterer(map, markers,
                    {
                        gridSize: 80,
                        imagePath: '../../Scripts/markercluster/images/m'
                    });
